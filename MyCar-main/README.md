...
```text
SpringWeb/
├── src/main/java/com/web/mycar/SpringWeb/
│   ├── contollers/       # Controladores (Logica de navegação e API)
│   │   ├── CarrosController.java
│   │   ├── ClientesController.java
│   │   ├── FuncionariosController.java
│   │   ├── HomeController.java      # Lógica do Dashboard
│   │   ├── PecasController.java
│   │   └── ServicosController.java  # Lógica complexa da OS e PDF
│   ├── models/           # Entidades JPA (Banco de Dados)
│   │   ├── Cliente.java
│   │   ├── Servico.java
│   │   ├── ServicoPeca.java  # Tabela pivô (Serviço <-> Peça)
│   │   └── ... (Outras models)
│   └── repository/       # Camada de acesso a dados (Queries personalizadas)
└── src/main/resources/
    ├── static/           # Assets (JS, CSS, Imagens)
    └── templates/        # Telas HTML (Thymeleaf)
        ├── carros/
        ├── clientes/
        ├── compartilhado/ # Fragmentos (Topo e Rodapé reutilizáveis)
        ├── financeiro/
        ├── funcionarios/
        ├── home/
        ├── pecas/
        └── servicos/      # Telas de OS e Modais
```
...
- 
### 📝 Cadastros (CRUDs Completos)
- **Clientes:** Cadastro com validação de dados.
- **Veículos:** Associação automática com clientes e exclusão em cascata (se apagar o carro, apaga os serviços vinculados para manter a integridade).
- **Peças:** Gestão de preços (compra/venda) e quantidade.
- **Funcionários:** Cadastro de mecânicos e equipe.
...
**Status**: ✅ Em desenvolvimendo em constante evolução e pronto para uso
